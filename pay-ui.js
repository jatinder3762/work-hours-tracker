window.initPayPeriodUI=()=>{
  ...
  renderWork=function(){baseRenderWork();updateWorkSummary()};
  ...
  renderPay=async function(){
    ...
    if(error){$('payPeriods').innerHTML='<p class="auth-error">Unable to load pay periods.</p>';return}
    ...
    ws(workplaceId).forEach(shift=>{
      ...
      if(!ranges.has(key))ranges.set(key,{period_start:range.start,period_end:range.end,paid:false});
    });
    const periods=[...ranges.values()].sort((a,b)=>b.period_start.localeCompare(a.period_start));
    $('payPeriods').innerHTML='';
    periods.slice(0,24).forEach(item=>{
